...
def parse_instance(content, outdir):
    ...
    merged_info_list = list(map(cytoolz.merge, cytoolz.join('id', content['images'], 'image_id',
                                                            content['annotations'])))
    ...
    for index, (name, groups) in enumerate(names_groups):
        ...
        assert not name.lower().startswith(("http:",
                                            "https:")), "Image seems to be a url rather than local. Need to set 'download_images' = False"
